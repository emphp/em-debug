...
use EM;
use em\base\Application;
use em\base\BootstrapInterface;
use em\helpers\Json;
use em\web\Response;
use em\helpers\Html;
use em\helpers\Url;
use em\web\View;
use em\web\ForbiddenHttpException;

/**
 * The Em Debug Module provides the debug toolbar and debugger
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @since 2.0
 */
class Module extends \em\base\Module implements BootstrapInterface
{
     ...
     * @inheritdoc
     */
    public $controllerNamespace = 'em\debug\controllers';
    /**
     * @var LogTarget
     */
    public $logTarget;
     ...
     */
    public static function getEmLogo()
    {
        return self::$_EmLogo;
    }
     ...
     */
    public static function setEmLogo($logo)
    {
        self::$_EmLogo = $logo;
    }
    ...
    {
        parent::init();
        $this->dataPath = EM::getAlias($this->dataPath);
        $this->initPanels();
    }
    ...
    protected function initPanels()
    {
        ...
        foreach ($this->panels as $id => $config) {
            ...
            $config['module'] = $this;
            $config['id'] = $id;
            $this->panels[$id] = EM::createObject($config);
        }
    }
    ...
    {
        $this->logTarget = EM::$app->getLog()->targets['debug'] = new LogTarget($this);
    ...
    public function beforeAction($action)
    {
        if (!$this->enableDebugLogs) {
            foreach (EM::$app->getLog()->targets as $target) {
        ...
        EM::$app->getView()->off(View::EVENT_END_BODY, [$this, 'renderToolbar']);
        EM::$app->getResponse()->off(Response::EVENT_AFTER_PREPARE, [$this, 'setDebugHeaders']);
     ...
     *
     * @param \em\base\Event $event
     * @since 2.0.7
     */
    public function setDebugHeaders($event)
    {
        if (!$this->checkAccess() || !EM::$app->getRequest()->getIsAjax()) {
            return;
        }
        ...
        $event->sender->getHeaders()
            ...
            ->set('X-Debug-Duration', number_format((microtime(true) - EM_BEGIN_TIME) * 1000 + 1))
    ...
    public function getToolbarHtml()
    {
        ...
        return '<div id="Em-debug-toolbar" data-url="' . Html::encode($url) . '" style="display:none" class="Em-debug-toolbar-bottom"></div>';
    }
    ...
    public function renderToolbar($event)
    {
        ...
        /* @var $view View */
        $view = $event->sender;
        echo $view->renderDynamic('return Em::$app->getModule("debug")->getToolbarHtml();');
    ...
    {
        $ip = EM::$app->getRequest()->getUserIP();
        ...
        EM::warning('Access to debugger is denied due to IP address restriction. The requesting IP address is ' . $ip, __METHOD__);
        return false;
    }
    ...
    {
        return [
            'config' => ['class' => 'em\debug\panels\ConfigPanel'],
            'request' => ['class' => 'em\debug\panels\RequestPanel'],
            'log' => ['class' => 'em\debug\panels\LogPanel'],
            'profiling' => ['class' => 'em\debug\panels\ProfilingPanel'],
            'db' => ['class' => 'em\debug\panels\DbPanel'],
            'assets' => ['class' => 'em\debug\panels\AssetPanel'],
            'mail' => ['class' => 'em\debug\panels\MailPanel'],
            'timeline' => ['class' => 'em\debug\panels\TimelinePanel']
        ];
    }
    ...
    protected function defaultVersion()
    {
        ...
        if (isset(EM::$app->extensions[$extensionName])) {
            return EM::$app->extensions[$extensionName]['version'];
        }
